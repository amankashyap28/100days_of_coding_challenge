...
class Solution{
    ...
    pair check(Node root){
        if(root == null) return new pair(true, Integer.MAX_VALUE);
        
        pair left = check(root.left);
        pair right = check(root.right);
        
        if(left.check && right.check){
            if((root.data == left.data || left.data == Integer.MAX_VALUE) && (root.data == right.data || right.data == Integer.MAX_VALUE)){
                cnt++;
